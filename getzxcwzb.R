############################################################
# Get latest financial data(chinese) from hexun
#
...
getzxcwzb <- function(stockid,date="latest",todataframe=F,na=F){
  # attach the package XML if it is not in the search path
  if(!("package:XML" %in% search()))
    library(XML)
  
  url <- paste("http://stockdata.stock.hexun.com/2009_zxcwzb_",stockid,".shtml",sep="")
  ...
  if(date == "latest"){
    ...
    url <- paste("http://stockdata.stock.hexun.com/2008/zxcwzb.aspx?stockid=",stockid,
