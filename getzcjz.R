############################################################
# Get Impairment of Asset from hexun.com
#
...
getzcjz <- function(stockid,date="latest",todataframe=F,na=F){
  # attach the package XML if it is not in the search path
  if(!("package:XML" %in% search()))
    library(XML)
  
  url <- paste("http://stockdata.stock.hexun.com/2009_zcjz_",stockid,".shtml",sep="")
  ...
  if(date == "latest"){
    ...
    url <- paste("http://stockdata.stock.hexun.com/2008/zcjz.aspx?stockid=",stockid,
